...
function applyTheme(theme: 'dark' | 'light') {
  ...
  if (theme === 'dark') { html.classList.add('dark'); html.style.backgroundColor = '#090909'; }
...
function MethodPill({ method }: { method: 'GET' | 'POST' }) {
  return (
    <span className="inline-flex items-center px-2 py-0.5 text-[10px] font-mono font-bold border"
      style={{ background: '#000', color: '#fff', borderColor: '#000', letterSpacing: '0.04em' }}>
      {method}
    </span>
  );
}
...
function LiveDot() {
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-mono font-bold border"
      style={{ border: '1px solid var(--status-success)', color: 'var(--status-success)', background: 'var(--status-success-bg)' }}>
...
function StatusBadge({ status }: { status: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-mono font-bold border"
      style={{ color: c, background: bg, borderColor: bd }}>
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-lg overflow-hidden"
        style={{ background: 'var(--panel)', border: '1px solid #000' }}
        onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-2.5 border-b" style={{ borderColor: '#000', background: '#000' }}>
...
function MusicTester({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className="px-2 py-1 border" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)' }}>
          <span className="text-[9px] font-mono font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>STYLE / DESCRIPTION</span>
        </div>
...
function TTDLTester({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className="px-2 py-1 border" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)' }}>
          <span className="text-[9px] font-mono font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>MEDIA URL</span>
        </div>
        <div className="flex">
          <input type="url" value={url} onChange={(e) => setUrl(e.target.value)}
            ...
            style={{ background: 'var(--panel-inner)', border: '1px solid var(--border-medium)', borderRight: 'none', color: 'var(--text-primary)' }} />
...
function GenericGetTester({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="px-3 py-2 border text-[9px] font-mono break-all" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)', color: 'var(--text-muted)' }}>
  ...
  return (
    <>
      <div className="border glow-card" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel)' }}>
        ...
        <div className="px-4 py-2.5 border-b" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)' }}>
        ...
        <button onClick={() => setExpanded((p) => !p)}
          ...
          style={{ background: expanded ? '#000' : 'var(--btn-ghost)', color: expanded ? '#fff' : 'var(--text-secondary)' }}>
          <span>↳ {expanded ? 'CLOSE TESTER' : 'TRY IT OUT'}</span>
          {expanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
        </button>
        {expanded && (
          <div className="p-4 border-t space-y-3" style={{ borderColor: 'var(--border-medium)' }}>
            {renderTester()}
          </div>
        )}
      </div>
    ...
    <section id={id}>
      <div className="flex items-center gap-0 border mb-1" style={{ borderColor: 'var(--border-medium)' }}>
        ...
        <div className="flex-1 px-3 py-2">
          <div className="flex items-center gap-2">
            <span className="text-sm font-mono font-bold uppercase tracking-wider" style={{ color: 'var(--text-primary)' }}>{title}</span>
            <span className="px-1.5 py-0 text-[9px] font-mono font-bold border"
              style={{ border: '1px solid var(--border-medium)', color: 'var(--text-muted)', background: 'var(--panel-inner)' }}>
              {count}
            </span>
          </div>
        ...
        {!loading && chartData.every((d) => d.count === 0) ? (
          ...
          <ResponsiveContainer width="100%" height={90}>
            <BarChart data={chartData} barSize={22}>
              ...
              <Tooltip contentStyle={{ background: '#000', border: '1px solid rgba(255,255,255,0.15)', fontSize: '10px', fontFamily: 'monospace', color: '#fff', borderRadius: 0 }} cursor={{ fill: 'rgba(0,0,0,0.04)' }} />
              <Bar dataKey="count" fill="var(--text-primary)" radius={0} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
    ...
    <div className="flex flex-col h-full">
      <div className="border-b" style={{ borderColor: 'var(--border-medium)', padding: '12px 16px' }}>
        <div className="flex items-center gap-2">
          <DaminiLogo size={22} />
          <div>
            <p className="text-xs font-mono font-bold uppercase tracking-wider" style={{ color: 'var(--text-primary)' }}>DAMINĪ API</p>
            <p className="text-[9px] font-mono" style={{ color: 'var(--text-muted)' }}>v2.0 · {totalEndpoints} endpoints</p>
          </div>
        </div>
      </div>
      <div className="border-b px-4 py-2.5 flex items-center gap-2" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)' }}>
        <span className="w-1.5 h-1.5 rounded-full pulse-active" style={{ background: 'var(--status-success)' }} />
        <span className="text-[9px] font-mono font-bold uppercase tracking-widest" style={{ color: 'var(--status-success)' }}>STATUS: 200 / ONLINE</span>
      </div>
      ...
      <div className="border-t" style={{ borderColor: 'var(--border-medium)' }}>
        <button onClick={() => setContactOpen(true)}
          className="w-full flex items-center gap-2 px-4 py-2.5 border-b text-left transition-colors hover:bg-black hover:text-white"
        ...
        <button onClick={toggleTheme}
          className="w-full flex items-center gap-2 px-4 py-2.5 text-left transition-colors hover:bg-black hover:text-white"
  ...
  return (
    <div className="flex min-h-screen" style={{ background: 'var(--canvas)', color: 'var(--text-primary)' }}>
      ...
      <main className="flex-1 min-w-0">
        ...
        {/* Topbar */}
        <div className="sticky top-0 z-40 border-b" style={{ background: 'var(--header-bg)', borderColor: 'var(--border-medium)', backdropFilter: 'blur(20px)' }}>
          <div className="flex items-center gap-3 px-4 h-12">
            <button className="lg:hidden flex items-center gap-1.5 border px-2 py-1 text-[9px] font-mono font-bold uppercase"
              style={{ borderColor: 'var(--border-medium)', color: 'var(--text-muted)' }}
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs font-mono focus:outline-none"
                style={{ background: 'var(--panel-inner)', border: '1px solid var(--border-medium)', color: 'var(--text-primary)' }} />
            </div>
            <button onClick={copyBase} className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 text-[9px] font-mono font-bold uppercase border transition-colors hover:bg-black hover:text-white"
              style={{ border: '1px solid var(--border-medium)', color: copiedBase ? 'var(--status-success)' : 'var(--text-muted)' }}>
              {copiedBase ? <><CheckCheck className="w-3 h-3" />COPIED</> : <><Copy className="w-3 h-3" />BASE URL</>}
            </button>
            <div className="ml-auto flex items-center gap-2">
              <button onClick={toggleTheme} className="w-7 h-7 border flex items-center justify-center transition-colors hover:bg-black hover:text-white"
                style={{ border: '1px solid var(--border-medium)', color: 'var(--text-muted)' }}>
                {theme === 'dark' ? <Sun className="w-3 h-3" /> : <Moon className="w-3 h-3" />}
              </button>
              <button onClick={() => setContactOpen(true)}
                className="flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-mono font-bold uppercase border transition-colors hover:bg-black hover:text-white"
        ...
        <div className="p-4 max-w-3xl space-y-6">
          ...
          {(isOverview || search) && (
            <>
              <div className="border" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel)' }}>
                <div className="flex items-center px-4 py-2 border-b" style={{ borderColor: 'var(--border-medium)', background: '#000' }}>
                  <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-white/60">BASE URL</span>
                  <div className="ml-auto flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full" style={{ background: 'var(--status-success)' }} />
                    <span className="text-[9px] font-mono text-white/40">{totalEndpoints} endpoints live</span>
                  </div>
                </div>
                <div className="flex items-center px-4 py-3 gap-3">
                  <code className="flex-1 text-sm font-mono font-bold break-all" style={{ color: 'var(--text-primary)' }}>{dynamicBase}</code>
                  <button onClick={copyBase} className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 border text-[9px] font-mono font-bold uppercase transition-colors hover:bg-black hover:text-white"
                    style={{ border: '1px solid var(--border-medium)', color: copiedBase ? 'var(--status-success)' : 'var(--text-muted)' }}>
                    {copiedBase ? <><CheckCheck className="w-3 h-3" />COPIED</> : <><Copy className="w-3 h-3" />COPY</>}
                  </button>
                </div>
                <div className="border-t px-4 py-2" style={{ borderColor: 'var(--border-medium)', background: 'var(--panel-inner)' }}>
                  <p className="text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>
                    Production API by <span style={{ color: 'var(--text-primary)' }} className="font-bold">Dev Daminī</span> · Damini Codesphere
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-4 border" style={{ borderColor: 'var(--border-medium)' }}>
                {[
                  { label: 'TOTAL',      value: statsLoading ? '—' : stats.total_requests.toLocaleString(), icon: Activity,      accent: undefined },
                  { label: 'EFFICIENCY', value: statsLoading ? '—' : `${stats.success_rate}%`,              icon: TrendingUp,    accent: undefined },
                  { label: 'SUCCESS',    value: statsLoading ? '—' : stats.total_success.toLocaleString(),  icon: CheckCircle2,  accent: 'success' as const },
                  { label: 'ERRORS',     value: statsLoading ? '—' : stats.total_errors.toLocaleString(),   icon: AlertTriangle, accent: 'error' as const },
                ].map(({ label, value, icon: Icon, accent }, i) => {
                  const col = accent === 'success' ? 'var(--status-success)' : accent === 'error' ? 'var(--status-error)' : 'var(--text-primary)';
                  return (
                    <div key={label} className={`px-3 py-4 ${i < 3 ? 'border-r' : ''}`}
                      style={{ borderColor: 'var(--border-medium)', background: 'var(--panel)' }}>
                      <Icon className="w-3.5 h-3.5 mb-2" style={{ color: col }} />
                      <p className="text-xl font-mono font-bold leading-none" style={{ color: col }}>{value}</p>
                      <p className="text-[9px] font-mono uppercase tracking-widest mt-1" style={{ color: 'var(--text-muted)' }}>{label}</p>
                    </div>
                  );
                })}
              </div>
